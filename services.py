...
def _get_visible_category_usage(username: str, month_label: str = "") -> dict[str, int]:
    ...
    if dataframe.empty:
        return {}
    usage: dict[str, int] = {}
    for category in dataframe["category"].dropna().astype(str).tolist():
...
def get_category_items(username: str, month_label: str = "") -> list[dict]:
    ...
    for normalized, count in usage.items():
        if normalized in items_by_key:
            continue
        display_name = next(
            (category for category in get_visible_expenses(get_expenses(), username)["category"].dropna().astype(str).tolist() if _normalize_category_name(category) == normalized),
            normalized.title(),
        )
